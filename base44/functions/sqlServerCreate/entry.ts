  ...
  try {
    ...
    return await new Promise((resolve) => {
      ...
      const request = new Request(
        `INSERT INTO ${tableName} (${columns.join(', ')}) VALUES (${placeholders}); SELECT SCOPE_IDENTITY() AS id;`,
        (err, rowCount) => {
          if (err) {
            resolve(Response.json({ error: err.message }, { status: 500 }));
          } else {
            resolve(Response.json({ success: true, insertedId: result?.id }));
          }
          connection.close();
        }
      );
      ...
      request.on('row', (columns) => {
        ...
        if (row.id) result = row;
      });

      columns.forEach((col) => {
        request.addParameter(col, values[columns.indexOf(col)]);
      });
