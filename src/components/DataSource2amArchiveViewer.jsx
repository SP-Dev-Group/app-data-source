...
export default function DataSource2amArchiveViewer({ open, onOpenChange }) {
  ...
  useEffect(() => {
    if (open) {
      setLoading(true);
      base44.entities.DataSource2amArchive.list("-created_date", 200).then((data) => {
        setRecords(data);
        setLoading(false);
      });
    }
  ...
  // Find the latest entry per source_id to know if it's currently deleted
  ...
  const reinstate = async (r) => {
    setReinstating(true);
    const newRecord = await base44.entities.DataSource2am.create({
      unique_id: r.unique_id,
      name: r.name,
      email: r.email || "",
    });
    const existing = records.filter((x) => x.source_id === r.source_id);
    await base44.entities.DataSource2amArchive.create({
    ...
    base44.entities.DataSource2amArchive.list("-created_date", 200).then(setRecords);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Archive — DataSource2am</DialogTitle>
        </DialogHeader>
        <Input
          ...
          {loading ? (
            ...
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-border bg-card">
                ...
                {filtered.length === 0 && (
                  <tr>
                    <td colSpan={7} className="px-3 py-8 text-center text-muted-foreground">No archive records found.</td>
                  </tr>
                )}
              </tbody>
            </table>
