...
export default function DataSourceRefresh5minsArchiveViewer({ open, onOpenChange }) {
  ...
  useEffect(() => {
    if (open) {
      setLoading(true);
      base44.entities.DataSourceRefresh5minsArchive.list("-created_date", 200).then((data) => {
        setRecords(data);
        setLoading(false);
      });
    }
  ...
  const reinstate = async (r) => {
    setReinstating(true);
    const newRecord = await base44.entities.DataSourceRefresh5mins.create({
      unique_id: r.unique_id,
      name: r.name,
    });
    const existing = records.filter((x) => x.source_id === r.source_id);
    await base44.entities.DataSourceRefresh5minsArchive.create({
    ...
    base44.entities.DataSourceRefresh5minsArchive.list("-created_date", 200).then(setRecords);
  };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Archive — DataSourceRefresh5mins</DialogTitle>
        </DialogHeader>
        <Input
