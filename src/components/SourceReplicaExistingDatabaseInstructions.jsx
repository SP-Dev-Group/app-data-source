      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-4 text-xs mt-4 whitespace-pre-wrap font-mono leading-relaxed text-foreground">
            {instructionContent.split(/(EntityNameReplica|sync"   "ToSourceListener|\{REPLICA_"  "_APP_ID\}|\{push"  "ToReplica\}|\{sync"  "ToSourceListener\}|unique_id)/g).map((part, idx) => 
              (part === 'EntityNameReplica' || part === 'sync"   "ToSourceListener' || part === '{REPLICA_"  "_APP_ID}' || part === '{push"  "ToReplica}' || part === '{sync"  "ToSourceListener}' || part === 'unique_id') ? 
