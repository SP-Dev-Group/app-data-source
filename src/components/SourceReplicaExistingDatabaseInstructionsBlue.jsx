...
function CopyableCode({ children }) {
  ...
  return (
    <div className="relative group">
      <pre className="bg-black text-blue-400 p-3 rounded text-[10px] overflow-x-auto max-h-96 overflow-y-auto pr-10">{children}</pre>
      <button
        onClick={handleCopy}
        className="absolute top-2 right-2 text-blue-400 hover:text-white transition-colors opacity-60 group-hover:opacity-100"
...
export default function SourceReplicaExistingDatabaseInstructionsBlue() {
        ...
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="border border-blue-200 rounded-lg p-3 bg-blue-50/50 space-y-2">
            ...
            <div className="flex gap-2">
              ...
              />
              <Button size="sm" variant="outline" onClick={handleSearch} disabled={searching} className="h-7 text-xs px-3 border-blue-300 text-blue-700 hover:bg-blue-50">
                <Search className="w-3 h-3 mr-1" /> Search
              </Button>
            </div>
            <p className="text-[10px] text-blue-500">All saved configs shown. Search to filter.</p>
            {showResults && (
              <div className="border border-blue-200 rounded bg-background text-xs max-h-40 overflow-y-auto">
                {searching ? (
                  ...
                  searchResults.map((r) => (
                    <div
                      key={r.id}
                      className="flex items-center justify-between px-3 py-1.5 hover:bg-blue-50 cursor-pointer border-b last:border-0"
          ...
          <div className="border border-blue-200 rounded-lg p-3 space-y-3">
            <div className="mb-2">
              <span className="text-xs font-bold text-white bg-blue-600 px-3 py-1 rounded">Step 2: Fill in Form</span>
            </div>
              ...
              <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
                ...
                <div className="space-y-4">
                  <div className="border rounded-lg p-3 bg-blue-50 border-blue-200 flex items-center justify-between">
                    <p className="text-xs text-blue-800 font-medium">Click and Copy this message and paste to the chat message Base44 in the Source App</p>
                    <Button
                      size="sm"
                      variant="outline"
                      onClick={() => {
...
Make it active, so it can run when triggered.`;
                  ...
                  <div className="border rounded-lg p-3 space-y-3">
                    ...
                    <p className="text-xs font-semibold text-blue-700 bg-blue-50 border border-blue-200 rounded p-2 mb-2">Instructions for the SOURCE APP, this app.</p>
                    ...
                    <div className="text-xs space-y-2 text-blue-700 bg-blue-50 border border-blue-200 rounded p-2">
                      ...
                      <p><strong>Instruction 3:</strong> Do not create the replica entity <code className="bg-blue-100 px-1 rounded">{form.replica_entity_name}</code> in this app, as this is the source app. The replica entity will be created in the external app using other code, not available here.</p>
                      <p><strong>Instruction 4:</strong> Use / reference only the source entity <code className="bg-blue-100 px-1 rounded">{form.source_entity_name}</code> and the replica entity <code className="bg-blue-100 px-1 rounded">{form.replica_entity_name}</code> named in the code, therefore, do not use another entity from within this app or the external app.</p>
                      <p><strong>Instruction 5:</strong> Use only the Replica Secret Name <code className="bg-blue-100 px-1 rounded">{form.secret_name}</code> and Replica App ID <code className="bg-blue-100 px-1 rounded">{form.replica_app_id}</code>, as provided in these notes and code blocks.</p>
                    </div>
                    <div className="text-xs space-y-2 text-muted-foreground border-t pt-2 mt-2">
                      <p className="font-semibold text-blue-700">2 STEPS for the SOURCE APP, this app.</p>
                      <p><strong>Step 1:</strong> Create a new backend function and call it <code className="bg-black text-blue-400 px-1 rounded">{form.sync_function_name}</code>. (Do not use existing backend functions in this app under any circumstance, do not use this code to overwrite any other function already existing in this app).</p>
                      <p><strong>Step 2:</strong> Create the other function: For the push function, use the push function name provided which is <code className="bg-black text-blue-400 px-1 rounded">{form.push_function_name}</code>. Do not use any other push function or function that may already exist in this app. Do not use this push function code to overwrite any other function or code already in this app.</p>
                    </div>
                    <div className="mt-3">
                  ...
                  <div className="border rounded-lg p-3 space-y-3">
                    <p className="text-xs font-bold text-blue-600 tracking-wide mb-2">SOURCE</p>
                    <p className="text-xs font-bold text-blue-600 mb-2">STEP 4</p>
                    <div className="text-xs space-y-2 text-foreground">
                      <p><strong>Make an automation</strong></p>
                      <p><strong>Name it:</strong> <code className="bg-black text-blue-400 px-1 rounded">{form.sync_function_name}{getTodayAUS()}</code></p>
                      <p><strong>Use:</strong> <code className="bg-black text-blue-400 px-1 rounded">{form.push_function_name}</code></p>
                      <p><strong>Runs When:</strong> <code className="bg-black text-blue-400 px-1 rounded">{form.source_entity_name}</code> is Created, Updated or Deleted.</p>
...
${generateSubscriptionCode(form.replica_entity_name)}`;
            ...
            <div className="flex items-center justify-between">
              <p className="text-xs font-semibold text-blue-700 uppercase tracking-wide">Config Fields</p>
            </div>

            <div className="flex flex-col gap-1">
              <p className="text-[10px] text-muted-foreground italic">provide a unique name that describes i.e "WorkersNotes4"</p>
              <div className="flex items-center gap-2">
                <span className="text-xs text-muted-foreground w-40 shrink-0">Project Name <span className="text-blue-500">*</span></span>
                <Input
            ...
            <Button
              ...
              onClick={handleSave}
              disabled={saving || !form.project_name.trim()}
              className="w-full h-7 text-xs mt-2 bg-blue-600 hover:bg-blue-700 text-white"
            >
              {saved ? <><Check className="w-3 h-3 mr-1" /> Saved!</> : saving ? "Saving..." : <><Save className="w-3 h-3 mr-1" /> Save Config</>}
            </Button>
            ...
            <div className="border border-blue-200 rounded-lg p-3 bg-blue-50/30">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 text-sm font-bold text-white uppercase tracking-wide px-3 py-2 rounded bg-blue-500 border border-blue-500 hover:bg-blue-600 transition-colors cursor-pointer"
                >
                  SOURCE
                </button>
                <button
                  onClick={() => setReplicaPopupOpen(true)}
                  className="flex-1 text-sm font-bold text-white uppercase tracking-wide px-3 py-2 rounded bg-blue-700 border border-blue-700 hover:bg-blue-800 transition-colors cursor-pointer"
                >
                  REPLICA
                </button>
              </div>
            ...
            <div className="border rounded-lg p-3 bg-blue-50 border-blue-200">
              <div className="mb-2">
                <span className="text-xs font-bold text-white bg-blue-600 px-3 py-1 rounded">Step 5: Test the Source and Replica function</span>
              </div>
              <p className="text-[10px] text-blue-700">After setting up both apps, test the synchronization by creating or updating records in the Source app and verifying they appear in the Replica app.</p>
            </div>
          </div>

          {/* Copy Reference Panel */}
          <div className="border border-blue-200 rounded-lg p-3 space-y-2 bg-blue-50/20">
            <p className="text-xs font-semibold text-blue-700 uppercase tracking-wide">Copy Reference Values</p>
