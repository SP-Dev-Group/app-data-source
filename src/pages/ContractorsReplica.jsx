...
import { Copy, Plus, Pencil, Trash2, Check, X } from "lucide-react";
import ContractorsReplicaInstructions from "@/components/ContractorsReplicaInstructions";
import PageMeta from "@/components/PageMeta";

export default function ContractorsReplica() {
  ...
  const createSample = async () => {
    ...
    const names = ["Alpha Contractor", "Beta Contractor", "Gamma Contractor", "Delta Contractor", "Epsilon Contractor"];
    const name = names[Math.floor(Math.random() * names.length)];
    await base44.entities.ContractorsReplica.create({ unique_id: uid, name });
    setCreating(false);
    loadRecords();
  };
  ...
  const loadRecords = () => {
    setLoading(true);
    base44.entities.ContractorsReplica.list().then((data) => {
      setRecords(data);
      setLoading(false);
    });
  };
  ...
  const saveEdit = async (r) => {
    ...
    setSaving(true);
    await base44.entities.ContractorsReplica.update(r.id, { name: editName.trim() });
    cancelEdit();
    loadRecords();
    setSaving(false);
  };
  ...
  const doDelete = async (id) => {
    setDeleting(true);
    await base44.entities.ContractorsReplica.delete(id);
    setDeleteConfirmId(null);
    loadRecords();
    setDeleting(false);
  };
    ...
    <div className="min-h-screen bg-background flex flex-col relative">
      ...
      <div className="absolute top-8 left-6">
        <div className="bg-black rounded-lg px-4 py-2">
          <span className="text-purple-400 font-medium">Contractors Replica</span>
        </div>
        <div className="flex items-center gap-2 mt-1">
          <p className="text-xs font-mono text-muted-foreground">Entity: ContractorsReplica</p>
          <button
            onClick={() => {
              navigator.clipboard.writeText("ContractorsReplica");
            }}
      ...
      <div className="flex flex-1 overflow-hidden">
          ...
          <div className="max-w-xl mx-auto pt-20">
            <div className="text-center mb-4">
              <div className="bg-purple-600 rounded-lg px-6 py-3 inline-block">
                <h1 className="text-3xl font-light tracking-tight text-white">Contractors Replica</h1>
              </div>
              <p className="text-sm font-light text-muted-foreground mt-3">Cross-app data synchronization</p>
            </div>
        ...
        <div className="w-56 border-l border-border bg-muted/20 p-4 flex flex-col gap-2 overflow-y-auto">
          ...
          <ContractorsReplicaInstructions />
          <PageMeta
            page="ContractorsReplica.jsx"
            functions={["pushToContractorsReplica"]}
            automations={[
              "Contractors Source to Replica Sync → pushToContractorsReplica (entity: ContractorsReplica, create/update)"
            ]}
            entities={[
              { name: "ContractorsReplica", type: "base44" }
            ]}
          />
          <Button
            variant="outline"
